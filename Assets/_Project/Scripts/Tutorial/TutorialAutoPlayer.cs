...
namespace SP.Tutorial
{
    ...
    public partial class TutorialAutoPlayer : MonoBehaviour
    {
        ...
        int pasosOk, pasosFallidos, pasosSaltados;
        ...
        public static TutorialAutoPlayer Lanzar(int desdePaso = 0)
        {
            var go = new GameObject("_TutorialAutoPlayer_Runtime");
            var c = go.AddComponent<TutorialAutoPlayer>();
            c.DesdePaso = desdePaso;
            c.StartCoroutine(c.Reproducir());
            return c;
        }
        ...
        IEnumerator Reproducir()
        {
            ...
            // Tabla: Id del paso -> gesto real (o null = usar SaltarPaso como reserva).
            while (tm != null && !tm.Terminado && tm.Indice >= 0 && tm.Indice < tm.Total)
            {
                ...
                // Espera a que el propio TutorialManager (con su Update/Evaluar de siempre)
                // reconozca el gesto y avance de paso solo, hasta un limite de tiempo.
                float limite = Time.time + (id == "avanzar_disparar" || id == "final" || id == "entrar_tanque" ? 100f : TimeoutPorPaso);
                while (tm.Indice == indiceAntes && Time.time < limite) yield return null;

                float dur = Time.time - tPasoInicio;
                if (tm.Indice > indiceAntes || tm.Terminado)
                {
                    if (gestoReal) { pasosOk++; Log($"[AUTOPLAY]   OK (gesto real) paso '{id}' avanzo en {dur:0.0}s"); }
                    else { pasosSaltados++; Log($"[AUTOPLAY]   SALTADO paso '{id}' (sin gesto real automatizado; se marco cumplido con SaltarPaso) en {dur:0.0}s"); }
                    resumen.AppendLine($"  {indiceAntes + 1,2}. {id,-18} {(gestoReal ? "GESTO REAL" : "SALTADO")}  {dur:0.0}s");
                }
                else
                {
                    pasosFallidos++;
                    Log($"[AUTOPLAY]   TEST FALLIDO: el paso '{id}' no avanzo en {TimeoutPorPaso:0}s pese al gesto. Se fuerza con SaltarPaso para no trabar la reproduccion.");
            ...
            Log("==================================================================");
            Log($"[AUTOPLAY] TUTORIAL COMPLETO en {total:0.0}s. OK(gesto real)={pasosOk}  Saltados={pasosSaltados}  Fallidos={pasosFallidos}  Total pasos={totalPasos}");
