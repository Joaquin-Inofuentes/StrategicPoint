...
models = [
    {"name": "Arbol 1", "label": "green"},
    {"name": "Arbol 3", "label": "green"},
...
for model in models:
    ...
    if os.path.isdir(model_dir):
        for f in os.listdir(model_dir):
            if f.endswith(".png") and "BaseMap" in f:
    ...
    if basemap_color:
        ...
        jsx_code += f"""
    try {{
        var group = doc.layerSets.getByName("{model_name}");
        groupLayers(group, "{model['label']}");
        
        var baseMapLayer = group.artLayers.getByName("BaseMap");
        doc.activeLayer = baseMapLayer;
        
        // Create solid color layer
        var colorLayer = createSolidColorLayer({r}, {g}, {b}, "Trimsheet Color (Multiply)");
...
with open("script_colors.jsx", "w", encoding="utf-8") as f:
    f.write(jsx_code)

print("Generated script_colors.jsx")
