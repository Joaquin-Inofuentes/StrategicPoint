...
|---|---|---|---|
| `SoldierMotor.cs` | 361 | SoldierMotor | Mueve el transform. Lo usan por igual el jugador y la IA detrás de la misma llamada, sin que a ninguno le importe quién … |
...
## Ai — 10 scripts, 2,819 lineas

> Cerebro de una unidad no poseida y el driver que avanza la simulacion.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `AiBrain.cs` | 1238 | CombatStance, AiBrain | Postura de combate de una unidad. Libre es el comportamiento historico y por defecto: las otras dos NO son un camino de … |
...
## Combat — 12 scripts, 2,449 lineas
...
| `WeaponHolder.cs` | 738 | WeaponChangedEvent, WeaponHolder | Un solo evento para "este soldado tiene otra arma puesta", sin importar el camino (recogida del piso, ciclado 1/2/3, IA)… |
...
| `ReinicioDeEstaticos.cs` | 23 | ReinicioDeEstaticos | Con "Enter Play Mode sin recarga de dominio" los estaticos sobreviven entre una corrida y la otra (modo dios, regeneraci… |
...
| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `HeadlessTestRunner.cs` | 4197 | HeadlessTestRunner, BenchResult, StressResult | Construye el entorno de prueba (suelo, obstáculos, prefabs, 3 soldados, cámara, UI, pool de proyectiles) y corre las 3 f… |
...
## Mision — 5 scripts, 1,201 lineas

> La partida: objetivos, oleadas, rehen, helicoptero, cinematica final.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `MisionDirector.cs` | 496 | FaseDeMision, MisionDirector | MISION DE RESCATE (partida principal). 1. INFILTRAR   avanzar entre las lineas enemigas hasta el CENTRO (la plaza de la … |
...
| `EstadoDePartida.cs` | 111 | EstadoDePartida | Ronda 11 (puntos 1 y 20). Antes la derrota se decidia UNA sola vez, en la corrutina de muerte del soldado poseido, asi q… |
...
## Player — 21 scripts, 7,902 lineas

> Traduccion de intencion a ordenes: input, posesion, seleccion, ordenes.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `PlayerInputDriver.cs` | 3479 | PlayerInputDriver | Traduce teclado/ratón reales a los mismos métodos que usa el test automático. No decide nada nuevo: es el "pegamento" de… |
...
| `PlayerInputDriver.Vehiculo.cs` | 651 | PlayerInputDriver | PlayerInputDriver (parte): manejo de vehiculos (entrar, asientos, camara, ordenes con [T]). |
...
| `PlayerInputDriver.Interaccion.cs` | 123 | PlayerInputDriver | PlayerInputDriver (parte): detector central de IInteractable. Archivo NUEVO y aditivo: no reemplaza nada de PlayerInputD… |
...
| `MunicionPickup.cs` | 115 | MunicionPickup | Municion suelta al morir un enemigo (item nuevo). 100% por codigo, sin prefab ni asset externo -- mismo espiritu que Caj… |
...
## Presentation — 56 scripts, 11,427 lineas
...
| `SoldierAnimatorDriver.cs` | 258 | SoldierAnimatorDriver | Traduce lo que el soldado YA hace a parametros del Animator. No decide nada: no mueve, no dispara, no cambia de estado. … |
...
| `MenuAmbiente.cs` | 134 | MenuAmbiente | Ambiente del menu principal (item 21): musica de fondo y un fondo tactico animado (mapa con cuadricula que se desplaza y… |
...
## Tutorial — 12 scripts, 4,483 lineas

> Modulo de ensenanza y su reproductor automatico.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `TutorialManager.cs` | 1883 | TutorialManager, Sub, Paso | Modulo de tutorial. Recorre 35 pasos en orden; cada paso tiene sub-pasos y cada sub-paso una BANDERA booleana (ver Tutor… |
...
| `WeaponStatusView.cs` | 219 | WeaponStatusView | HUD fijo (no depende de apuntar a nada) con qué arma tenés, cuánta munición te queda y una barra de recarga/enfriamiento… |
...
## Vehicles — 10 scripts, 2,370 lineas
