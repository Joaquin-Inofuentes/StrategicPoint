# Mapa de archivos
> 221 scripts. Generado el 2026-09-20 07:30 UTC.

## Actors — 4 scripts, 612 lineas

> El soldado: identidad, piezas, motor de movimiento y aspecto.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `SoldierMotor.cs` | 314 | SoldierMotor | Mueve el transform. Lo usan por igual el jugador y la IA detrás de la misma llamada, sin que a ninguno le importe quién … |
...
## Ai — 9 scripts, 2,709 lineas

> Cerebro de una unidad no poseida y el driver que avanza la simulacion.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `AiBrain.cs` | 1233 | CombatStance, AiBrain | Postura de combate de una unidad. Libre es el comportamiento historico y por defecto: las otras dos NO son un camino de … |
| `AiBrain.Navegacion.cs` | 413 | AiBrain | AiBrain (parte): rutas, rodeo de obstaculos, deteccion de trabas y avance hacia un punto. |
| `AiBrain.Tactica.cs` | 375 | AiBrain | Parte "tactica" del cerebro: coberturas (ordenadas por el jugador o buscadas por el propio enemigo en combate), seguir a… |
...
| `WorldSimulationDriver.cs` | 163 | WorldSimulationDriver | Avanza IA, armas y vehículos cada frame en Play mode real. El test automático no usa esto: simula el mismo paso a mano p… |
...
| `AjustesDeEscuadra.cs` | 59 | AjustesDeEscuadra | Ajustes de la escuadra aliada, editables desde el Inspector. GameObject: "Systems/AjustesDeEscuadra" (componente Ajustes… |
| `AiState.cs` | 15 | AiState | — |

## Camera — 2 scripts, 869 lineas

> Rig de camara: hombro, RTS, transiciones, sacudidas, zoom.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `CameraRig.cs` | 823 | ControlMode, CameraRig | Posee la cámara y delega su posición en el modo activo (FPS u RTS). |
...
## Combat — 12 scripts, 2,438 lineas
...
| `WeaponHolder.cs` | 735 | WeaponChangedEvent, WeaponHolder | Un solo evento para "este soldado tiene otra arma puesta", sin importar el camino (recogida del piso, ciclado 1/2/3, IA)… |
...
| `Health.cs` | 170 | Health | Única responsabilidad: llevar la cuenta de puntos de vida y avisar cuando cambian o llegan a cero. No sabe que existe un… |
...
| `ProjectilePool.cs` | 115 | ProjectilePool | Dueño del pool de proyectiles. Cero Instantiate en combate: todo pasa por acá. El pool en sí (ObjectPool<T>) es estado d… |
...
| `NavService.cs` | 258 | NavService | EL PEGAMENTO QUE FALTABA. El proyecto ya tenia WaypointGraph (A*) y FlowField, escritos y cubiertos por la suite headles… |
...
| `Loc.cs` | 182 | Idioma, Loc, LocTraductor | Localizacion (item 27), espanol -> ingles. El texto original del juego es la CLAVE (no hay tablas de ids que mantener): … |
...
| `ReinicioDeEstaticos.cs` | 22 | ReinicioDeEstaticos | Con "Enter Play Mode sin recarga de dominio" los estaticos sobreviven entre una corrida y la otra (modo dios, regeneraci… |
...
## Editor — 30 scripts, 13,984 lineas

> Herramientas de Editor: suite headless, constructores de escena, pipelines de arte.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `HeadlessTestRunner.cs` | 4195 | HeadlessTestRunner, BenchResult, StressResult | Construye el entorno de prueba (suelo, obstáculos, prefabs, 3 soldados, cámara, UI, pool de proyectiles) y corre las 3 f… |
| `HeadlessTestRunner.Fases8a12.cs` | 1546 | HeadlessTestRunner | HeadlessTestRunner (parte): fases 8 a 12 de la suite. |
...
| `ArtBuilder.cs` | 785 | ArtBuilder, Volumen, PropDef | Segunda mitad del pipeline de arte: con los FBX ya importados y materializados por ArtSetup, esto arma lo que el juego u… |
...
| `TutorialSceneBuilder.cs` | 435 | TutorialSceneBuilder, Cubo | Arma SC_Tutorial: una escena NUEVA hecha a partir de SC_Gameplay (asi trae el jugador, los aliados, el tanque, el HUD, l… |
| `HeadlessTestRunner.Fase19.cs` | 406 | HeadlessTestRunner | FASE 19 (ronda 9, segunda tanda): ayudas del tutorial, radial legible, boton de menu uniforme, zoom, salto agachado, ord… |
...
| `HeadlessTestRunner.Fase18.cs` | 129 | HeadlessTestRunner | FASE 18 (ronda 9): reservas de municion, mando, accesibilidad, fuego amigo en DIFICIL, IA con granadas y supresion, esta… |
...
| `PlaymodeCleanup.cs` | 86 | PlaymodeCleanup | BUG REAL: decals (agujeros de bala, crateres) y escombros creados en Play mode quedaban manchando la escena despues de f… |
...
## Mision — 4 scripts, 1,083 lineas

> La partida: objetivos, oleadas, rehen, helicoptero, cinematica final.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `MisionDirector.cs` | 489 | FaseDeMision, MisionDirector | MISION DE RESCATE (partida principal). 1. INFILTRAR   avanzar entre las lineas enemigas hasta el CENTRO (la plaza de la … |
...
## Player — 21 scripts, 7,720 lineas

> Traduccion de intencion a ordenes: input, posesion, seleccion, ordenes.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `PlayerInputDriver.cs` | 3470 | PlayerInputDriver | Traduce teclado/ratón reales a los mismos métodos que usa el test automático. No decide nada nuevo: es el "pegamento" de… |
| `OrderService.cs` | 694 | FormationKind, OrderService | Disposiciones posibles de un lote de destinos. Vive AFUERA de OrderService (que es una clase estatica) para que el drive… |
| `PlayerInputDriver.Vehiculo.cs` | 646 | PlayerInputDriver | PlayerInputDriver (parte): manejo de vehiculos (entrar, asientos, camara, ordenes con [T]). |
| `PlayerInputDriver.Radial.cs` | 575 | PlayerInputDriver | PlayerInputDriver (parte): menu radial de [Q] y ejecucion de las ordenes de escuadra. |
| `Demolicion.cs` | 319 | Demolicion, DemoledorAsalto | DEMOLER (habilidad del soldado de ASALTO): plantar una carga en un muro o cobertura. Hay que estar AGACHADO y QUIETO fre… |
| `PedidoDeCuracion.cs` | 289 | PedidoDeCuracion | "Necesito curarme" del menu de ordenes ([Q] sostenido). De las cinco ordenes del menu, cuatro ya existian en OrderServic… |
...
| `SelectionController.cs` | 213 | SelectionController | Selección múltiple en vista RTS. |
...
| `RescateAutomatico.cs` | 104 | RescateAutomatico | A5: "un aliado libre va a revivirte y frena el timer". Mismo patron que PedidoDeCuracion (curarse por pedido del jugador… |
| `PlayerBrain.cs` | 99 | PlayerBrain | La consciencia que salta de cuerpo en cuerpo. Traduce intención en llamadas al soldado que ocupa. Es único en la escena. |
...
| `MunicionPickup.cs` | 92 | MunicionPickup | Municion suelta al morir un enemigo (item nuevo). 100% por codigo, sin prefab ni asset externo -- mismo espiritu que Caj… |
| `PlayerInputDriver.Interaccion.cs` | 72 | PlayerInputDriver | PlayerInputDriver (parte): detector central de IInteractable. Archivo NUEVO y aditivo: no reemplaza nada de PlayerInputD… |
| `PlayerInputDriver.Escuadra.cs` | 53 | PlayerInputDriver | PlayerInputDriver (parte): orden de la escuadra. El lugar en la lista fija a que numero de roster y a que [F1..F9] respo… |
...
## Presentation — 52 scripts, 10,745 lineas

> Todo lo que se ve y se oye en el mundo: VFX, audio, marcadores, pools visuales.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `AudioDirector.cs` | 645 | SfxChannel, VoiceState, AudioDirector | Canales de mezcla (item 186). Van a nivel de namespace y no anidados en AudioDirector por la misma razon que SfxKind viv… |
| `ImpactFx.cs` | 617 | ImpactFx, FxMode, ImpactFxPool | Mini-explosion al impactar: una esfera que se agranda rapido y despues se achica hasta desaparecer, en el punto exacto d… |
...
| `WorldUiDirector.cs` | 387 | WorldUiDirector | Un unico recorrido para TODA la UI de mundo (barras de vida, marcador de poseido, iconos de minimapa) mas nivel de detal… |
| `MinimapIcon.cs` | 313 | MinimapIcon | Circulo chato que representa a un soldado/vehiculo en el minimapa. Vive en su propia capa (Minimap), que la cámara princ… |
...
| `CubeFxReactor.cs` | 264 | CubeFxReactor | Único puente entre el bus de eventos y lo que se ve/oye de un soldado. No decide nada de gameplay: solo reacciona. Se au… |
...
| `KillFeedbackDirector.cs` | 233 | KillFeedbackDirector | Un solo lugar que decide como se comunica una baja. Estaba repartido (el kill feed apilaba lineas por su cuenta, la miri… |
| `ObstacleMarker.cs` | 221 | ObstacleMarker | Marca un cubo como "obstáculo" para que un proyectil lo detecte y el jugador tenga feedback distinto al pegarle a un obs… |
...
| `CoverHologram.cs` | 211 | CoverHologram | Holograma de cobertura. Mientras el jugador mantiene [Shift] apuntando a una cobertura se muestra, ESTATICO y 90 % trans… |
...
| `SoldierAnimatorDriver.cs` | 208 | SoldierAnimatorDriver | Traduce lo que el soldado YA hace a parametros del Animator. No decide nada: no mueve, no dispara, no cambia de estado. … |
...
| `GameOutcomeController.cs` | 189 | GameOutcomeController | Pantallas de victoria y derrota: UI distinta para cada una (colores, texto) pero los mismos dos botones -- Reintentar (r… |
| `UnitLabelView.cs` | 185 | UnitLabelView | C1: etiqueta al pie de cada unidad con su vida, su tipo (aliado / enemigo / vehiculo / interactuable) y, si es montable,… |
...
| `GameplaySceneBootstrap.cs` | 162 | GameplaySceneBootstrap | Marca en el log de flujo que la escena de gameplay terminó de cargar (Start corre después de que todo el resto ya se con… |
...
| `DecalPool.cs` | 144 | DecalKind, DecalPool | Marcas persistentes en el terreno (crateres de explosion y agujeros de bala). Mismo problema que los escombros: sin un c… |
| `MainMenuController.cs` | 141 | MainMenuController | Menú de inicio: [Jugar] carga la escena de gameplay, [Salir] cierra el juego (o sale de Play mode si esto corre en el Ed… |
...
| `RevivePromptView.cs` | 137 | RevivePromptView | Cartel mundial "[Q] REVIVIR" sobre un aliado caido. Mismo patron que UnitLabelView (Canvas WorldSpace propio, billboard … |
...
| `MenuAmbiente.cs` | 121 | MenuAmbiente | Ambiente del menu principal (item 21): musica de fondo y un fondo tactico animado (mapa con cuadricula que se desplaza y… |
...
| `AnuncioDeZonas.cs` | 83 | AnuncioDeZonas, Zona | Avisa en que bloque del nivel esta el jugador (el nivel mide 320 m de largo: sin esto no hay sensacion de avance). Al en… |
...
| `ImpactCubes.cs` | 57 | ImpactSurface, ImpactCubes | Impacto de un proyectil, en cubitos (pedido: "si es soldado cubitos de distintos tamaños rojos, si es piso verde y si es… |
...
## Tutorial — 12 scripts, 4,480 lineas

> Modulo de ensenanza y su reproductor automatico.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `TutorialManager.cs` | 1880 | TutorialManager, Sub, Paso | Modulo de tutorial. Recorre 35 pasos en orden; cada paso tiene sub-pasos y cada sub-paso una BANDERA booleana (ver Tutor… |
...
| `TutorialAutoPlayer.Radial.cs` | 298 | TutorialAutoPlayer | Gestos de las ordenes radiales, la seleccion en FPS, el tanque y la torreta fija. La mira es real (se gira al soldado y … |
...
| `MenuDeOrdenes.cs` | 625 | ContextoRadial, MenuDeOrdenes | Contexto del radial: que categorias y opciones tienen sentido AHORA segun lo que se apunta. Las contextuales (curar al h… |
| `MinimapFollow.cs` | 338 | MinimapFollow | Cámara cenital del minimapa: sigue al objetivo actual (soldado o vehículo poseído) desde arriba, mirando siempre hacia a… |
...
| `MirillaView.cs` | 285 | MirillaView | La mirilla que se ve al mantener el clic derecho (apuntar con zoom). Antes el "zoom" agrandaba x4 el anillo de la mira (… |
| `ControlsTable.cs` | 281 | ControlContext, ControlEntry, ControlsTable | Los seis contextos de entrada reales. Son flags porque un mismo atajo puede leerse en varios (por ejemplo [TAB], que se … |
...
| `RosterRowView.cs` | 248 | RosterRowView | Una fila del roster = un prefab por soldado. Pedido explicito: "un prefab para cada caso... con puntero al soldado... si… |
| `AjustesDeJuego.cs` | 243 | AjustesDeJuego, PanelAjustesExtra | Opciones que no tenian donde vivir: pantalla completa, resolucion, calidad, paleta para daltonismo y HUD minimo. Se guar… |
| `WeaponStatusView.cs` | 213 | WeaponStatusView | HUD fijo (no depende de apuntar a nada) con qué arma tenés, cuánta munición te queda y una barra de recarga/enfriamiento… |
...
| `OffscreenAllyMarkerView.cs` | 198 | OffscreenAllyMarkerView | Item 63: marcas en el borde apuntando a los aliados que quedaron fuera de encuadre. En primera persona el jugador perdia… |
...
| `TurretAimView.cs` | 170 | TurretAimView | HUD propio del artillero. Como artillero se usaba la misma mirilla de infanteria, que no comunica lo unico que importa e… |
...
| `OffscreenKillMarkerView.cs` | 115 | OffscreenKillMarkerView | Las bajas fuera de encuadre solo llegaban por el feed de texto, que se pierde entre el resto de la informacion. Una flec… |
...
| `ModoDiosView.cs` | 66 | ModoDiosView | Cartel fijo "MODO DIOS [F4]" mientras nadie de tu bando recibe dano: se ve siempre para que nadie juegue una partida ent… |
...
| `HudPulido.cs` | 60 | HudPulido | Ajustes de HUD que valen para TODAS las escenas de juego (gameplay, tutorial, pruebas) sin tocar cada constructor de esc… |
...
| `RosterView.cs` | 53 | RosterView | Arma el roster instanciando UN prefab de fila (RosterRowView) por soldado del equipo del jugador -- pedido explicito: "u… |
...
## Vehicles — 10 scripts, 2,317 lineas

> Tanque, asientos, torretas y ametralladoras fijas.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `Vehicle.cs` | 648 | Vehicle | Vehículo con 4 asientos: conductor, artillero y dos pasajeros. Los soldados que suben quedan ocultos (su cuerpo se desac… |
| `TurretWeapon.cs` | 440 | TurretWeapon, AmmoType | Arma montada en el vehículo: gira sola (no con el chasis) y dispara proyectiles del mismo pool que las armas de mano. |
...
| `VehicleMotor.cs` | 159 | VehicleMotor | Motor del vehículo: acelera y frena de forma progresiva (no es instantáneo como el motor de un soldado), y gira más rápi… |
| `Atropello.cs` | 109 | Atropello | Atropellar con el vehiculo en movimiento. KnockNearbyProps ya hacia este barrido para los objetos livianos (bidones, caj… |
