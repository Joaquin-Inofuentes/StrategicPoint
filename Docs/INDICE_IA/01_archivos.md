# Mapa de archivos
> 254 scripts. Generado el 2026-09-22 14:57 UTC.
...
## Combat — 12 scripts, 2,490 lineas
...
| `WeaponHolder.cs` | 768 | WeaponChangedEvent, WeaponHolder | Un solo evento para "este soldado tiene otra arma puesta", sin importar el camino (recogida del piso, ciclado 1/2/3, IA)… |
...
| `NavService.cs` | 262 | NavService | EL PEGAMENTO QUE FALTABA. El proyecto ya tenia WaypointGraph (A*) y FlowField, escritos y cubiertos por la suite headles… |
...
| `SceneLoader.cs` | 24 | SceneLoader | Unico punto de entrada para cambiar de escena en el juego real. Antes cada pantalla (menu, victoria/derrota, pausa, tuto… |
...
## Editor — 40 scripts, 16,198 lineas

> Herramientas de Editor: suite headless, constructores de escena, pipelines de arte.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `HeadlessTestRunner.cs` | 4249 | HeadlessTestRunner, BenchResult, StressResult | Construye el entorno de prueba (suelo, obstáculos, prefabs, 3 soldados, cámara, UI, pool de proyectiles) y corre las 3 f… |
| `HeadlessTestRunner.Fases8a12.cs` | 1551 | HeadlessTestRunner | HeadlessTestRunner (parte): fases 8 a 12 de la suite. |
| `ArtSetup.cs` | 1012 | ArtSetup, Prop, RecalibrarAlturaAlReimportar | Pipeline de importacion del arte de Assets/ARTS. Es una herramienta y no un README con pasos a mano a proposito: el arte… |
...
| `ArtBuilder.cs` | 818 | ArtBuilder, Volumen, PropDef | Segunda mitad del pipeline de arte: con los FBX ya importados y materializados por ArtSetup, esto arma lo que el juego u… |
...
| `BlockoutArtDresser.cs` | 467 | BlockoutArtDresser | "Implementa arte en todo": viste el BLOCKOUT (cubos grises) con los prefabs de arte de Assets/_Project/Prefabs/ArteMundo… |
...
| `NightLightingBuilder.cs` | 262 | NightLightingBuilder | Pedido explicito: "quiero que de la imagen de noche, una noche medio iluminada por lamparas y estrellas en el cielo" + "… |
...
| `MenuSceneBuilder.cs` | 197 | MenuSceneBuilder | Arma la escena de menú principal: fondo, título, botón Jugar y botón Salir. Mismo patrón que HeadlessTestRunner: todo po… |
...
| `HeadlessTestRunner.BusquedasGlobales.cs` | 158 | HeadlessTestRunner | Red de seguridad del plan "eliminar los Find de runtime": el codigo de runtime solo puede BAJAR su cantidad de busquedas… |
...
| `LoadingSceneBuilder.cs` | 113 | LoadingSceneBuilder | Arma la escena intermedia de carga: nada mas que fondo oscuro, un texto "CARGANDO" y una barra de progreso. Mismo patron… |
...
| `PlaymodeCleanup.cs` | 90 | PlaymodeCleanup | BUG REAL: decals (agujeros de bala, crateres) y escombros creados en Play mode quedaban manchando la escena despues de f… |
...
## Mision — 5 scripts, 1,309 lineas

> La partida: objetivos, oleadas, rehen, helicoptero, cinematica final.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `MisionDirector.cs` | 578 | FaseDeMision, MisionDirector | MISION DE RESCATE (partida principal). 1. INFILTRAR   avanzar entre las lineas enemigas hasta el CENTRO (la plaza de la … |
| `CinematicaDeVictoria.cs` | 283 | CinematicaDeVictoria | Cinematica de victoria: la escuadra y el civil suben al helicoptero, este despega en una nube de polvo y huye hacia el o… |
| `Helicoptero.cs` | 198 | Helicoptero | El helicoptero de extraccion (geometria P_Veh_Heli del arte): helices que giran, disco de desenfoque a tope de vueltas, … |
...
## Presentation — 61 scripts, 12,346 lineas
...
| `PauseController.cs` | 463 | PauseController | Pausa con [ESC]: congela el tiempo (Time.timeScale=0) y muestra el panel de pausa, con un sub-panel de "Configuraciones"… |
...
| `CubeFxReactor.cs` | 288 | CubeFxReactor | Único puente entre el bus de eventos y lo que se ve/oye de un soldado. No decide nada de gameplay: solo reacciona. Se au… |
...
| `KillCylinderFx.cs` | 246 | KillCylinderFx | Marca el lugar donde cae un enemigo con un cilindro delgado que sube muy por encima del mapa y se desvanece solo. Mismo … |
...
| `MainMenuController.cs` | 173 | MainMenuController | Menú de inicio: [Jugar] carga la escena de gameplay, [Salir] cierra el juego (o sale de Play mode si esto corre en el Ed… |
...
| `Feedback.cs` | 166 | Feedback, WorldTag | Punto unico de feedback de las ACCIONES del juego (pedido: "toda accion tenga feedback visual y auditivo"). Cada accion … |
...
| `TrayectoriaGranadaView.cs` | 145 | TrayectoriaGranadaView | Vista previa de la granada mientras se MANTIENE [G]: la curva exacta (la misma simulacion que usa la granada de verdad, … |
...
| `LimpiezaDeEscena.cs` | 124 | LimpiezaDeEscena | Del plan del usuario: "Al re cargar la escena no se limpian los decals". Es cierto y no era solo de los decals. Los tres… |
...
| `ObjectiveArrowIndicator.cs` | 100 | ObjectiveArrowIndicator | Pedido explicito: "en la base del soldado una flecha que indique hacia donde ir" -- un decal plano en el suelo, pegado a… |
...
| `LoadingScreenController.cs` | 64 | LoadingScreenController | Unico contenido de SC_Loading: una barra que se llena mientras la escena destino (SP.Core.SceneLoader.Destino) se carga … |
...
## Tutorial — 13 scripts, 4,564 lineas
...
| `TutorialUI.cs` | 358 | TutorialUI | Cuadro de dialogo del tutorial (columna derecha, debajo del minimapa): TUTORIAL · PASO 3 / 17 DISPARAR                  … |
...
## UI — 43 scripts, 7,638 lineas

> HUD y pantallas: mira, roster, minimapa, radial, menus, ajustes.

| Archivo | Lineas | Tipos | Que hace |
|---|---|---|---|
| `AimUI.cs` | 711 | AimUI | Retículo + prompt contextual: "F: Poseer a X" o "T: Ir aquí". También resalta el retículo cuando el disparo del jugador … |
...
| `ButtonSfx.cs` | 110 | ButtonSfx | Pedido explicito: "todos los botones deben tener un sonido al apuntar al boton y otro sonido al hacerle click". Un solo … |
...
| `MilitaryButtonSkin.cs` | 103 | MilitaryButtonSkin | Pedido explicito: "imagen low-poly futurista belico" para el fondo de los botones, y "mejorar el arte de la UI" en gener… |
...
## Vehicles — 11 scripts, 2,472 lineas
...
| `TorretaFija.cs` | 256 | TorretaFija | AMETRALLADORA FIJA: los emplazamientos del mapa (P_Env_Emplazamiento_MG, en las torres y los bunkers) dejan de ser decor… |
...
| `HitboxDeImpacto.cs` | 16 | HitboxDeImpacto | Marca un collider que existe SOLO para que le puedan pegar -- no es un obstaculo real. NavService.BlocksMovement lo excl… |
