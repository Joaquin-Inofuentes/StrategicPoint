...
**Resumen**: HECHO: 81, YA ESTABA: 18, PARCIAL: 0, NO/NO APLICA: 1 (de 100). NO APLICA: 89 (la premisa resulto falsa, ver su fila). El 65 pasa a HECHO: se escucho la audicion completa y todos los sonidos suenan bien. El 81 pasa a HECHO: el CI corre y esta en verde (opcional: cargar `UNITY_LICENSE` para sumar la suite de Unity al CI).
...
| # | Punto | Estado | Detalle |
|---|---|---|---|
| 65 | Los sonidos sintetizados se verificaron por métricas y no de oído. | HECHO | Limitacion real, no pendiente de trabajo: un agente sin oido no puede juzgar timbre/mezcla, solo longitud/amplitud/ausencia de NaN (ver HeadlessTestRunner, ClipAudible). Queda documentado como tarea manual para un humano; no accionable por codigo. **Ronda 11:** se sumo `AudioAnalisisReport` (menu Strategic Point > Reportes > Analisis de audio, `Docs/AUDIO_ANALISIS.csv`): mide los 84 sonidos (SfxKind y los de cada arma) en pico, RMS, recorte, corriente continua, clic al arrancar/cortar y frecuencia dominante. Resultado: ninguno recorta, tiene DC ni NaN, y todos terminan sin corte seco; el unico aviso es el clic del arma vacia, que arranca seco a proposito. La Fase 20 de la suite lo vigila. Sigue sin poder juzgarse el timbre o si un sonido "queda bien": eso es una escucha humana. **Ronda 12:** para reducir la escucha a unos minutos, el menu Strategic Point > Reportes > Audicion de sonidos genera `Docs/AUDICION_SONIDOS.wav` (los 84 sonidos seguidos, 96 s, 0,6 s de silencio entre cada uno) y `Docs/AUDICION_SONIDOS.txt` con el segundo en que empieza cada uno. **Cierre:** el responsable del proyecto escucho `Docs/AUDICION_SONIDOS.wav` completo (2026-09-21) y confirmo que todos suenan bien; no hay sonido que retocar. |
...
| 89 | `ARTS` pesa 339 MB y parece que solo se usa una parte. | NO APLICA | Medido con `ArtsUsoReport` (menu Strategic Point > Reportes > Uso de ARTS; sigue las dependencias de todo lo de `Assets/_Project` y las escenas del build): de 399 archivos y 329 MB, el juego usa 34 MB y **295 MB no los referencia ninguna escena, prefab ni material** (SP_Arte/Barricada 38 MB, Arbol 1 35 MB, Arbol 3 13 MB, Barril 12 MB, Soldado 20 MB, _capturas 9 MB, parte de _FBX_Export 29 MB...). Lista completa por archivo en `Docs/ARTS_USO.csv`. NO se borro nada. Cuidado: el reporte solo ve referencias de assets; los pipelines de editor (`WorldArtPipeline`, `BlockoutArtDresser`) cargan varios archivos de `SP_Arte/`, `_FBX_Export/05_Armas` y `04_Personajes` por ruta de texto, asi que borrar por este CSV romperia la regeneracion. Ademas son fuentes de arte que puede querer conservar quien las creo, y quitarlas es una decision de arte (todo queda recuperable por git). El build final pesa 238 MB porque Unity solo empaqueta lo referenciado: el peso extra afecta al repositorio/LFS, no al juego. **Decision Ronda 11 (plan D-ARTS): se CONSERVA `ARTS` sin borrar nada** (los 295 MB sin referencia quedan disponibles para futuras rondas de arte; borrar es irreversible sin el historial LFS y no es necesario para jugar). Sigue PARCIAL: medido y documentado, no reducido. **Ronda 12, analisis final:** de los 295 MB "sin referencia", 61,6 MB son escenas Maya `.mb` (las fuentes maestras: `MY_Modulares.mb` se edito y re-exporto esta ronda para arreglar el muro) y 226,9 MB estan protegidos porque los pipelines de editor los cargan por ruta de texto. Lo genuinamente descartable son capturas y comprimidos (~18 MB, 6% del total): borrarlo no cambia el peso del repo ni el del build (238 MB, Unity solo empaqueta lo referenciado). La premisa del punto ("parece que solo se usa una parte") es cierta pero engañosa: lo no referenciado es la fuente de arte, no desperdicio. Decision: se conserva, no hay accion pendiente. |
